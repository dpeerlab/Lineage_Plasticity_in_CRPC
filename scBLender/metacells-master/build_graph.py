...
from sklearn.neighbors import kneighbors_graph, radius_neighbors_graph, KNeighborsTransformer
...
class MetacellGraph:
    ...
    def rbf(self, k:int=15, metric = 'minkowski'):
        ...
        knn_graph = kneighbors_graph(self.Y, k, mode="connectivity", metric=metric, include_self=True) # kneighbors_graph(self.Y, k, mode="connectivity", metric=metric, include_self=True)
        knn_graph_distances = kneighbors_graph(self.Y, k, mode="distance", metric=metric, include_self=True) # kneighbors_graph(self.Y, k, mode="distance", metric=metric, include_self=True)
